...
function install_venv() {
    ...
    # Force version of pip for reproducibility, but there is nothing special
    # about this version.  Update whenever a new version is released and
    # verified functional.
    echo "There was a problem installing some dependencies, so I want to make sure
	that the latest version is installed, so i inserted different strategies, it can
	take longer to install the latest pip but i believe it is the safest way"
	  curl https://bootstrap.pypa.io/get-pip.py | "${VIRTUALENV_ROOT}/bin/${opt_python}" - 'pip==23.3.2'

	curl https://bootstrap.pypa.io/get-pip.py | "${VIRTUALENV_ROOT}/bin/${opt_python}" - "${VIRTUALENV_ROOT}/bin/pip" install pip==23.3.2

"${VIRTUALENV_ROOT}/bin/pip" install pip==23.3.2
    "${VIRTUALENV_ROOT}/bin/pip" install --upgrade pip==23.3.2
    # Upgrade pip to the latest version
    curl https://bootstrap.pypa.io/get-pip.py | "${VIRTUALENV_ROOT}/bin/${opt_python}" - 'pip' && "${VIRTUALENV_ROOT}/bin/pip" install --upgrade pip

# Vérifier la version actuelle de pip
current_pip_version=$("${VIRTUALENV_ROOT}/bin/pip" --version | awk '{print $2}')

# Comparer la version actuelle avec la version minimale requise
if [[ "$current_pip_version" < "23.3.2" ]]; then
    # Mettre à jour pip uniquement si la version actuelle est inférieure à 23.3.2
    echo 'Mise à jour de pip vers la version 23.3.2'
    "${VIRTUALENV_ROOT}/bin/pip" install --upgrade pip==23.3.2
fi
    echo "These dependencies caused problems, so this is an attempt to fix the problem."
    #"${VIRTUALENV_ROOT}/bin/pip" install "cython<3.0.0" && pip install --no-build-isolation pyyaml==6.0
	
# Installer "cython<3.0.0" et pyyaml==6.0
"${VIRTUALENV_ROOT}/bin/pip" install "cython<3.0.0"
"${VIRTUALENV_ROOT}/bin/pip" install --no-build-isolation pyyaml==6.0
